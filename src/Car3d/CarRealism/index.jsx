...
import {
  DataTexture, RGBAFormat, FloatType, ShaderChunk, ShaderLib, UniformsUtils,
  WebGLMultisampleRenderTarget, ShaderMaterial, GLSL3, NoBlending, Uniform, Vector2,
  Matrix4, Vector3, Clock, Quaternion, LinearFilter, HalfFloatType, FramebufferTexture,
  TextureLoader, NearestFilter, RepeatWrapping, NoColorSpace, WebGLRenderTarget, 
  ClampToEdgeWrapping, LinearMipMapLinearFilter, EquirectangularReflectionMapping, 
  Color, Matrix3, TangentSpaceNormalMap, RedFormat, Source, Texture, NoToneMapping, 
  PerspectiveCamera, SRGBColorSpace, DepthTexture
} from 'three';
...
class CopyPass extends Pass {
  constructor(textureCount = 1) {
    ...
    this.renderTarget = new WebGLMultisampleRenderTarget(1, 1, 1, {
      depthBuffer: false
    });
...
class TemporalReprojectPass extends Pass {
  constructor(scene, camera, velocityDepthNormalPass, textureCount = 1, options = defaultTemporalReprojectPassOptions) {
    ...
    this.renderTarget = new WebGLMultisampleRenderTarget(1, 1, textureCount, {
...
class PoissionDenoisePass extends Pass {
  constructor(camera, inputTexture, depthTexture, options = defaultPoissonBlurOptions) {
    ...
    this.renderTargetA = new WebGLMultisampleRenderTarget(1, 1, 2, renderTargetOptions);
    this.renderTargetB = new WebGLMultisampleRenderTarget(1, 1, 2, renderTargetOptions);
    const {
      uniforms
...
class SSGIPass extends Pass {
  constructor(ssgiEffect, options) {
    ...
    this.renderTarget = new WebGLMultisampleRenderTarget(1, 1, bufferCount, {
      type: HalfFloatType,
      depthBuffer: false
